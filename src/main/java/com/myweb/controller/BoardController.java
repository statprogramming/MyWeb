...
public class BoardController extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// MVC2 방식에서 페이지 이동은 기본적으로 포워드 형식을 사용한다
		// 리다이렉트는 다시 컨트롤러를 태울 때 사용한다.
		// 부모 인터페이스 선언
		BoardService service;

		if(command.equals("/board/list.board")) { // 3. 목록 진입(데이터 조회)
			...
			// response.sendRedirect("board_list.jsp"); // 나가는 경로
		...
		} else if(command.equals("/board/content.board")) { // 4. 상세 내용(화면에서 bno 넘겨준다)
		...
		} else if(command.equals("/board/modify.board")) { // 5. 수정 화면(화면에서 bno 넘겨준다)
			...
			response.sendRedirect("content.board?bno=" + request.getParameter("bno"));
			 ...
			 /*
	          * 1. 화면에 삭제 버튼에서는 delete.board요청으로 필요한 값을 넘겨준다.
	          * 2. DeleteServiceImpl() 생성하고 dao의 delete()메소드로 실행
	          * 3. 삭제 진행후에 목록페이지로 이동.
	          */
