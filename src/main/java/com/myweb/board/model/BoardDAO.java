...
public class BoardDAO {
	// 1. 싱글톤패턴을 이용한 클래스
	// 스스로 객체를 생성하고 1개로 제한
	private static BoardDAO dao = new BoardDAO(); // 1개
	// 2. 생성자에 private을 붙인다.
	private BoardDAO() {
		try {
	...
	public ArrayList<BoardVO> getList(int pageNum, int amount) {
		...
		ArrayList<BoardVO> list = new ArrayList<>(); // 셀렉트한 결과를 담을 리스트
		...
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, (pageNum - 1) * amount); // (조회하는 페이지 - 1) * 데이터 개수
			pstmt.setInt(2, pageNum * amount);
			
			rs = pstmt.executeQuery();
			while(rs.next()) {
				// 1. 모든 컬럼값을 가지고 와서(rs.getString - 스트링 타입으로 반환, rs.getInt - 정수 타입으로 반환, rs.getTimestamp - 시간형 반환)
	...
	public void update(String bno, String title, String content) {
		...
		try {
			conn = ds.getConnection();
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, title); // setInt(1, Integer.parseInt(bno));
